package ru.weatherclock.adg.app.presentation.components.calendar.styles
...
fun Modifier.getPointerCursor() = this.pointerHoverIcon(
    PointerIcon.Hand,
    true
)
...
class DateInputDefaults {
    ...
    data class DateInputBorders(
        val focusedBorder: BorderSpecs = BorderSpecs(
            2.dp,
            selected
        ),
        val simpleBorder: BorderSpecs = BorderSpecs(
            1.dp,
            enabled
        ),
        val withValueBorder: BorderSpecs = BorderSpecs(
            2.dp,
            enabled
        ),
    )
