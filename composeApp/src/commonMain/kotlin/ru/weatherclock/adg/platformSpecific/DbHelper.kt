...
object DbHelper {
    ...
    fun createAccuweatherDb(): AccuweatherDb {
        return AccuweatherDb(
            driver = PlatformHelper.createDbDriver(
                schema = AccuweatherDb.Schema, dbName = "accuweather.db"
            ),
            ...
            ForecastDetailAdapter = ForecastDetail.Adapter(
                intAdapter, intAdapter, intAdapter, intAdapter, intAdapter, intAdapter, intAdapter
            ),
    ...
    fun createProdCalendarDb(): ProdCalendarDb {
        return ProdCalendarDb(
            driver = PlatformHelper.createDbDriver(
                schema = ProdCalendarDb.Schema, dbName = "prod_calendar.db"
            ),
    ...
    fun createOpenWeatherMapDb(): OpenWeatherMapDb {
        return OpenWeatherMapDb(
            driver = PlatformHelper.createDbDriver(
                schema = OpenWeatherMapDb.Schema, dbName = "open_weather_map.db"
